...
PASSWORD_REGEX = re.compile (r'^[A-Z0-9.+_-]+$')
...
class User:
    ...
    def create_user(cls,user_data):
        data = {
            ...
            "dob":user_data['dob'],
            "favorite_language" :user_data['favorite_language'],
            "password":bcrypt.generate_password_hash(user_data['password'])
        }
        query = "INSERT INTO users (first_name, last_name, email, dob, favorite_language, password, created_at, updated_at) VALUES (%(first_name)s,%(last_name)s,%(email)s,%(dob)s,%(favorite_language)s,%(password)s,NOW(), NOW() );"
    ...
    def validate_reg(data, user):
        ...
        if not PASSWORD_REGEX.match(data['password']):
            flash("* At least 1 Upper case letter and 1 number are required in your password. ", 'register')
            is_valid = False
        if not data['dob']:
            flash("* Date of Birth is required.", 'register')
            is_valid = False
    ...
    def validate_login(login_data, user):
        ...
        if not user:
            ...
            flash("*E-mail and password combination do not match our records. Please try again. ", "login")
            is_valid = False
